...
These programs are made to be understandable to beginners, hence sometimes they
might do things in a round-about way for simplicity sake. (e.g. I don't ever use
list comprehensions, etc.)

My main questions are:
    1) What are the parts that are most confusing to understand?
    2) Is there enough commenting? Are there any comments that should be added?
    3) Are there bits of code that could be simplified or even removed? (I've previously
       found functions that I never call in previous reviews of the code.)
    4) Is it understandable by a novice programmer just by reading the code & comments?

These programs are meant to be run on Python 3 with Pygame, but I think they'll run on Python 2.
...
def main():
    global FPSCLOCK, WINDOWSURF, GEMIMAGES, GAMESOUNDS, BASICFONT, BOARDRECTS, debugsurf
    ...
    debugsurf = pygcurse.PygcurseSurface(20, 6)
    debugsurf.setscreencolors(clear=True)
    debugsurf.pygprint('Hello world!Hello world!Hello world!')
...
def runGame():
    ...
    while True: # main game loop
        ...
        if gameIsOver:
            ...
            lastScoreDeduction = time.time()
        drawScore(score)
        debugsurf.blitto(WINDOWSURF, (WINDOWWIDTH - debugsurf.pixelwidth - 20, WINDOWHEIGHT - debugsurf.pixelheight - 20))
